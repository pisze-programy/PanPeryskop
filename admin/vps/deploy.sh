...
echo "== push to $HOST =="
scp -r -q "$ROOT/backend/dist/seed-consumer.mjs" "$ROOT/backend/dist/kup-warm.mjs" "$ROOT/backend/dist/awin-warm.mjs" "$ROOT/backend/dist/travel.mjs" "$ROOT/admin/vps/consumer.sh" "$ROOT/admin/vps/consumer-ensure.sh" "$ROOT/admin/vps/setup-vps.sh" "$ROOT/admin/vps/ipv4-proxy.mjs" "$HOST:/tmp/"
echo "pushed"
