...
echo "== push to $HOST =="
scp -q "$ROOT/backend/dist/vps-seed.mjs" "$ROOT/backend/dist/kup-warm.mjs" "$ROOT/backend/dist/awin-warm.mjs" "$ROOT/admin/vps/orchestrator.sh" "$ROOT/admin/vps/setup-vps.sh" "$ROOT/admin/vps/ipv4-proxy.mjs" "$ROOT/admin/src/seed-ingest.mjs" "$HOST:/tmp/"
echo "pushed"
