...
import { produceProviderWindow } from '../seed/pipeline/queue/produce';
import { configOf } from '../seed/providers/registry';
...
// One-off Worker-only refill of a single provider's window units. Never touches
// another provider, so it costs no proxy data. Body: { provider }.
adminRoutes.post('/seed/provider', async (c) => {
  if (!adminAuth(c)) return c.json({ error: 'Forbidden' }, 403);
  const body = await c.req.json<{ provider?: unknown }>().catch(() => ({} as { provider?: unknown }));
  const id = typeof body.provider === 'string' ? body.provider : '';
  const cfg = configOf(id as ProviderId);
  if (!cfg || !cfg.enabled || cfg.executors.worker !== true || cfg.executors.vps !== undefined) {
    return c.json({ error: 'provider is not an enabled Worker provider' }, 400);
  }
  const res = await produceProviderWindow(c.env, id as ProviderId, warsawDateOf(Date.now()), 'manual');
  return c.json({ ok: true, ...res });
});

// One-off backfill of the shared club photo on every club night. The source
// carries no image of its own; the Worker default is the techno-party-club object.
const CLUB_MEDIA_KEY = 'posts/defaults/techno-party-club.jpg';
const CLUB_THUMB_KEY = 'posts/defaults/techno-party-club-thumb.jpg';
adminRoutes.post('/seed/ra-media', async (c) => {
  if (!adminAuth(c)) return c.json({ error: 'Forbidden' }, 403);
  const res = await c.env.DB
    .prepare(`UPDATE posts SET media_key=?, thumb_key=?, external_media_url=NULL, external_thumb_url=NULL
              WHERE external_id LIKE 'ra-%'`)
    .bind(CLUB_MEDIA_KEY, CLUB_THUMB_KEY)
    .run();
  return c.json({ ok: true, updated: Number(res?.meta?.changes ?? 0) });
});

// Manual city-break import. The list is generated by `npm run gen:cities` —
// re-run after a source change. Idempotent by city id.
...
// Our own photo or video for one city, replacing the hotlink. The 4090 job
// uploads the file to R2 and posts the public path here.
adminRoutes.post('/seed/cities/media', async (c) => {
  if (!adminAuth(c)) return c.json({ error: 'Forbidden' }, 403);
  const body = await c.req
    .json<{ id?: unknown; image?: unknown; imageLarge?: unknown; video?: unknown }>()
    .catch(() => ({} as { id?: unknown; image?: unknown; imageLarge?: unknown; video?: unknown }));
  const id = typeof body.id === 'string' ? body.id : '';
  if (!id) return c.json({ error: 'id required' }, 400);
  const image = typeof body.image === 'string' ? body.image : null;
  const imageLarge = typeof body.imageLarge === 'string' ? body.imageLarge : null;
  const video = typeof body.video === 'string' ? body.video : null;
  if (!image && !imageLarge && !video) return c.json({ error: 'image, imageLarge or video required' }, 400);
  await c.env.DB.prepare(
    `UPDATE travel_cities SET
       image_url = COALESCE(?, image_url),
       image_large_url = COALESCE(?, image_large_url),
       video_url = COALESCE(?, video_url)
     WHERE id = ?`,
  ).bind(image, imageLarge, video, id).run();
  return c.json({ ok: true, id, image, imageLarge, video });
});
