...
import { DAY_MS, SEED_REFILL_AHEAD } from '../core/constants';
import { todayWarsaw, addDaysWarsaw } from '../core/dates';

const SEED_RETENTION_MS = (SEED_REFILL_AHEAD + 6) * DAY_MS; // window + 6 days of audit
...
export async function pruneSeedData(env: Env, runType: RunType = 'cron'): Promise<PruneResult> {
  ...
  const windowEnd = addDaysWarsaw(todayWarsaw(), SEED_REFILL_AHEAD + 1);
