...
test('tagCatalog: explicit tag_order positions win, deleted tags are dropped', async () => {
  ...
  assert.deepEqual(ids.slice(2), ['filmy', 'muzyka', 'meetup', 'komedia', 'teatr', 'restauracje', 'inne'], 'canonical follows in default order');
...
test('registry: kupbilecik + ebilet + eventim + ticketmaster are the worker providers', async () => {
  ...
  assert.ok(ids.includes('maratonypolskie'), 'maratonypolskie runs on the worker');
  assert.deepEqual(ids, ['kupbilecik', 'ebilet', 'eventim', 'ticketmaster', 'residentadvisor', 'maratonypolskie']);
});
