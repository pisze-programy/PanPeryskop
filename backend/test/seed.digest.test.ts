...
test('activeSeedProviders: returns the 9 automated providers, excludes facebook + disabled', () => {
  const p = activeSeedProviders();
  assert.deepEqual(p, ['cinemacity', 'ebilet', 'eventim', 'going', 'helios', 'kupbilecik', 'luma', 'meetup', 'multikino'].sort());
...
test('recordSeedDigest: dedupes a same-status retry (one report)', async () => {
  ...
  try {
    ...
    assert.equal(mine[0].data?.job, '1/9');
  } finally {
    global.fetch = fetch;
  }
});
...
test('recordSeedDigest: day-done email fires once when all 9 providers report', async () => {
  const { reports, fetch } = capturedReports();
  try {
    const db = new MockDigestDB();
    const env = { DB: db, SNITCH_URL: 'https://cf-snitch.example', SNITCH_TOKEN: 't' };
    ...
    assert.equal(dayDone[0].data?.providers, 9);
  } finally {
    global.fetch = fetch;
  }
});
