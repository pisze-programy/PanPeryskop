...
test('doSavePost: persists is_sold_out flag on insert and update', async () => {
  ...
  assert.equal(ins!.binds[ins!.binds.length - 8], 1, 'is_sold_out=1 on insert');
  assert.equal(ins!.binds[ins!.binds.length - 7], '2026-08-17', 'event_date set from created_at (events only)');
  assert.equal(ins!.binds[ins!.binds.length - 6], null, 'showtimes NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 5], null, 'showtime_booking NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 4], null, 'tags NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 3], null, 'partner_id NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 2], null, 'partner_name NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 1], null, 'price_pln NULL by default');
...
test('doSavePost: live posts (no external_id) get event_date NULL', async () => {
  ...
  assert.equal(ins!.binds[ins!.binds.length - 7], null, 'live post event_date is NULL');
});
