...
test('doSavePost: persists is_sold_out flag on insert and update', async () => {
  ...
  // createdAt, cellId, sponsored, category, linkUrl, externalId, is_sold_out, event_date, showtimes, showtime_booking, tags, partner_id, partner_name).
  ...
  assert.equal(ins!.binds[ins!.binds.length - 7], 1, 'is_sold_out=1 on insert');
  assert.equal(ins!.binds[ins!.binds.length - 6], '2026-08-17', 'event_date set from created_at (events only)');
  assert.equal(ins!.binds[ins!.binds.length - 5], null, 'showtimes NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 4], null, 'showtime_booking NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 3], null, 'tags NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 2], null, 'partner_id NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 1], null, 'partner_name NULL by default');
  ...
  // thumb, sponsored, category, linkUrl, createdAt, externalId, is_sold_out, event_date, showtimes, showtime_booking, tags, partner_id, partner_name, id).
  calls.length = 0;
  ...
  assert.equal(upd!.binds[upd!.binds.length - 8], 0, 'is_sold_out=0 on update');
  assert.equal(upd!.binds[upd!.binds.length - 7], '2026-08-17', 'event_date updated');
  assert.equal(upd!.binds[upd!.binds.length - 6], null, 'showtimes NULL on update');
  assert.equal(upd!.binds[upd!.binds.length - 5], null, 'showtime_booking NULL on update');
  assert.equal(upd!.binds[upd!.binds.length - 4], null, 'tags NULL on update');
  assert.equal(upd!.binds[upd!.binds.length - 3], null, 'partner_id NULL on update');
  assert.equal(upd!.binds[upd!.binds.length - 2], null, 'partner_name NULL on update');
