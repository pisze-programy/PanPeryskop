...
test('doSavePost: persists is_sold_out flag on insert and update', async () => {
  ...
  // createdAt, cellId, sponsored, category, linkUrl, externalId, is_sold_out, event_date, showtimes, showtime_booking, tags, partner_id, partner_name, price_pln).
  ...
  assert.equal(ins!.binds[ins!.binds.length - 9], 1, 'is_sold_out=1 on insert');
  assert.equal(ins!.binds[ins!.binds.length - 8], '2026-08-17', 'event_date set from created_at (events only)');
  assert.equal(ins!.binds[ins!.binds.length - 7], null, 'showtimes NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 6], null, 'showtime_booking NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 5], null, 'tags NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 4], null, 'partner_id NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 3], null, 'partner_name NULL by default');
  assert.equal(ins!.binds[ins!.binds.length - 2], null, 'price_pln NULL by default');
  ...
  assert.ok(/partner_id/.test(ins!.sql), 'INSERT includes partner_id column');
  assert.ok(/partner_name/.test(ins!.sql), 'INSERT includes partner_name column');
  ...
  assert.equal(upd!.binds[upd!.binds.length - 11], 0, 'is_sold_out=0 on update');
  assert.equal(upd!.binds[upd!.binds.length - 10], '2026-08-17', 'event_date updated');
  assert.equal(upd!.binds[upd!.binds.length - 9], null, 'showtimes NULL on update');
  assert.equal(upd!.binds[upd!.binds.length - 8], null, 'showtime_booking NULL on update');
  assert.equal(upd!.binds[upd!.binds.length - 7], null, 'tags NULL on update');
  assert.equal(upd!.binds[upd!.binds.length - 6], null, 'partner_id NULL on update');
  assert.equal(upd!.binds[upd!.binds.length - 5], null, 'partner_name NULL on update');
  assert.equal(upd!.binds[upd!.binds.length - 4], null, 'price_pln NULL on update');
  assert.equal(upd!.binds[upd!.binds.length - 3], null, 'source_url NULL on update');
...
test('doSavePost: update promotes pending→approved (status bound) but preserves admin rejections', async () => {
  ...
  assert.equal(upd!.binds[13], 'approved', 'status column bound to the new status (pending→approved promotion)');
...
test('doSavePost: update preserves tags when tags_locked=1, applies the new tag when unlocked', async () => {
  ...
  assert.equal(upd!.binds[18], '["sport"]', 'tags JSON bound for the unlocked path');
});
