...
test('doSavePost: persists is_sold_out flag on insert and update', async () => {
  ...
  // createdAt, cellId, sponsored, category, linkUrl, externalId, is_sold_out, event_date, showtimes).
  ...
  assert.equal(ins!.binds[ins!.binds.length - 3], 1, 'is_sold_out=1 on insert');
  assert.equal(ins!.binds[ins!.binds.length - 2], '2026-08-17', 'event_date set from created_at (events only)');
  assert.equal(ins!.binds[ins!.binds.length - 1], null, 'showtimes NULL by default');
  ...
  // thumb, sponsored, category, linkUrl, createdAt, externalId, is_sold_out, event_date, showtimes, id).
  calls.length = 0;
  ...
  assert.equal(upd!.binds[upd!.binds.length - 4], 0, 'is_sold_out=0 on update');
  assert.equal(upd!.binds[upd!.binds.length - 3], '2026-08-17', 'event_date updated');
  assert.equal(upd!.binds[upd!.binds.length - 2], null, 'showtimes NULL on update');
