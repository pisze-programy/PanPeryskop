...
test('providers: going=fetch, kupbilecik=browser, all enabled (multikino disabled: CF egress 403)', () => {
  ...
  assert.equal(byId.get('multikino')!.enabled, false, 'multikino disabled (403 from Worker egress)');
